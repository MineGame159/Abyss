...
public class GpuSwapchain {
    ...
    public GpuImage? GetNextImage(Semaphore semaphore) {
        ...
        if (result != Result.Success && result != Result.SuboptimalKhr) throw new Exception("Failed to acquire next swapchain image");
    ...
    public unsafe void Present(GpuImage image, Semaphore waitSemaphore) {
        ...
        if (window.FramebufferSize.As<uint>() != FramebufferSize) Recreate(true);
    }

    private unsafe void Recreate(bool resize) {
        VkUtils.Wrap(ctx.Vk.DeviceWaitIdle(ctx.Device), "Failed to wait for device idle");
        ...
        var queueIndex = VkUtils.GetQueueIndices(ctx.Vk, ctx.PhysicalDevice).Graphics!.Value;
    ...
    public static implicit operator SwapchainKHR(GpuSwapchain swapchain) {
